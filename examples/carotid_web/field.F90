...
contains
  ...
  subroutine MakeRandomMesh(npoint, pts)
    integer :: npoint
    real(WP), dimension(npoint, 3) :: pts
    ...
    do p_it = 1, npoint
      len = RandomNumber(ranseed)*(maxval(wall%x(:, 3)) - minval(wall%x(:, 3))) + minval(wall%x(:, 3))
      maxN = -1E9
      minN = 1E9
      ...
      if (minN .eq. 1E9) write (*, *) "ERR - min not found"
      if (maxN .eq. -1E9) write (*, *) "ERR - max not found"

      minN = minN + thresh
      maxN = maxN - thresh
      if (minN .ge. maxN) write (*, *) "ERR - min > max"
  ...
  subroutine InitAll
    ...
    ! change restart file to input string
